...
def send_push_notification(nombre,titulo, msg):
    
    global API_KEY
    namespace_manager.set_namespace('-global-')

    q = AppConfig.all()
    q = db.GqlQuery("SELECT * FROM AppConfig")
    for p in q.run(limit=1):
        API_KEY = p.gcm_api_key
    ...
    if reg_id_list is None:
        ...
        logging.info('Lista: %s', reg_id_list)
        logging.info('Lista: %s', len(reg_id_list))
